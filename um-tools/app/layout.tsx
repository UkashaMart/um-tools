...
export const metadata: Metadata = {
  ...
  description: "Free browser-based tools. Word Counter, Zakat Calculator, BMI, QR Generator and more. No signup, no tracking.",
  keywords: "free tools, word counter, zakat calculator, bmi, qr code, pakistan tools",
  openGraph: {
    title: "UM Tools — Free Online Utility Tools",
  ...
  return (
    <html lang="en">
      <head>
        ...
        <script async src="https://www.googletagmanager.com/gtag/js?id=G-MT7QYETVNL" />
        <script dangerouslySetInnerHTML={{ __html: `window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-MT7QYETVNL');` }} />
        <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-7032253044947209" crossOrigin="anonymous" />
      </head>
      <body>
        <header style={{ background:'#111827', borderBottom:'1px solid rgba(212,160,23,0.4)', padding:'0 1.25rem', display:'flex', alignItems:'center', gap:'0.75rem', height:'52px', position:'sticky', top:0, zIndex:100 }}>
          <Link href="/" style={{ display:'flex', alignItems:'center', gap:'0.75rem', textDecoration:'none' }}>
            <div style={{ width:36, height:36, borderRadius:'50%', overflow:'hidden', flexShrink:0 }}>
              <img src="/logo.png" alt="UM Tools" width={36} height={36} style={{ width:'100%', height:'100%', objectFit:'cover' }} />
            </div>
            <div>
        ...
        <footer style={{ background:'#111827', borderTop:'1px solid rgba(212,160,23,0.2)', padding:'1.5rem', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', justifyContent:'center', gap:'1.5rem', marginBottom:'.75rem', flexWrap:'wrap' }}>
            {[['/', 'Home'],['/blog','Blog'],['/about','About'],['/contact','Contact'],['/privacy-policy','Privacy Policy']].map(([href,label])=>(
              <Link key={href} href={href} style={{ fontSize:'.78rem', color:'#9A8F78', textDecoration:'none' }}>{label}</Link>
            ))}
          </div>
          <div style={{ fontSize:'.72rem', color:'#9A8F78' }}>© {new Date().getFullYear()} UM Tools — ukashamart.com</div>
        </footer>
      </body>
    </html>
  );
