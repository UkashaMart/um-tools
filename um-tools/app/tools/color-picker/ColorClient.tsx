...
export default function ColorClient() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:".78rem", color:"#9A8F78", marginBottom:"1rem" }}>HEX, RGB, HSL colors convert karo - web designers aur developers ke liye free tool</p>
        ...
        {valid && (
          <>
            <div style={{ display:"flex", flexWrap:"wrap", gap:".6rem", marginBottom:".6rem" }}>
              {[[`${r}, ${g}, ${b}`,"RGB"],[`${h}deg, ${s}%, ${l}%`,"HSL"],[hex.toUpperCase(),"HEX"]].map(([v,k])=>(
      ...
      <div className="card" style={{ marginBottom:"1rem" }}>
        <h2 style={{ fontFamily:"Cinzel,serif", fontSize:"1rem", color:"#F5C842", marginBottom:".75rem" }}>Color Formats Explained</h2>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:".6rem" }}>
          {[["HEX","Web development mein sabse common","#D4A017"],["RGB","Red Green Blue values 0-255","rgb(212, 160, 23)"],["HSL","Hue Saturation Lightness","hsl(43, 80%, 46%)"]].map(([title,desc,ex])=>(
      ...
      <div className="card" style={{ marginBottom:"1rem" }}>
        <h2 style={{ fontFamily:"Cinzel,serif", fontSize:"1rem", color:"#F5C842", marginBottom:".75rem" }}>Popular Colors Pakistan</h2>
        <div style={{ display:"flex", flexWrap:"wrap", gap:".6rem" }}>
          {[["#01411C","Pakistan Green"],["#FFFFFF","White"],["#D4A017","Gold"],["#001F5A","Navy Blue"],["#C8102E","Red"],["#00A550","Emerald"]].map(([color,name])=>(
      ...
      <div className="card">
        ...
        {[["Color picker free hai?","Haan bilkul free hai. Koi signup nahi chahiye."],["HEX aur RGB mein kya fark hai?","HEX 6 digit code hai, RGB teen numbers hain. Dono same color represent karte hain."],["CSS mein kaunsa format best hai?","HEX sabse common hai, lekin HSL dark/light mode ke liye best hai."]].map(([q,a])=>(
